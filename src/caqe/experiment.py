    ...
    for c_id in condition_ids:
        ...
        if app.config['TEST_TYPE'] == 'segmentation':
            if app.config['STIMULUS_ORDER_RANDOMIZED']:
                random.shuffle(condition_group_data['clip_files'])
                random.shuffle(condition_data['clip_keys'])

            if app.config['ENCRYPT_AUDIO_STIMULI_URLS']:
                condition_group_data['clip_files'] = encrypt_audio_stimuli(condition_group_data['clip_files'],
                                                                           participant_id,
                                                                           condition.group_id)
                encoding_map, _, _ = get_encoding_maps(condition_group_data['stimulus_files'])
                condition_data['clip_keys'] = [encoding_map[key] for key in condition_data['clip_keys']]
        else:
            if app.config['STIMULUS_ORDER_RANDOMIZED']:
